...
const getFavicon = (url) => `https://www.google.com/s2/favicons?sz=128&domain=${url}`;

// --- State Management ---
let state = {
  settings: JSON.parse(localStorage.getItem('speeddial_settings')) || { ...DEFAULT_SETTINGS },
  tiles: JSON.parse(localStorage.getItem('speeddial_tiles')) || [...INITIAL_TILES],
  pages: JSON.parse(localStorage.getItem('speeddial_pages')) || [...INITIAL_PAGES],
...
const saveState = () => {
  try {
    ...
    if (e.name === 'QuotaExceededError') {
      alert("Local storage is full! This is usually caused by a very large background image file. Please use a smaller image (under 2MB) or host it online and use the URL instead.");
    } else {
...
const renderCalendar = () => {
  ...
  document.querySelectorAll('.cal-day').forEach(btn => {
    btn.onclick = () => {
      ...
      window.open(`https://calendar.google.com/calendar/u/0/r/day/${year}/${m}/${d}`, '_blank');
    };
  });
};
...
const fetchWeather = async () => {
  ...
  const results = await Promise.all(state.settings.weatherConfigs.map(async config => {
    try {
      const unit = state.settings.weatherUnit === 'imperial' ? '&temperature_unit=fahrenheit&wind_speed_unit=mph' : '';
      const res = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${config.lat}&longitude=${config.lng}&current=temperature_2m,weather_code,relative_humidity_2m,wind_speed_10m,is_day${unit}`);
      const data = await res.json();
      return { ...config, data: data.current };
    } catch { return { ...config, data: null }; }
  }));

  const renderWeatherHtml = (r) => {
    if (!r.data) return `<div class="p-4 rounded-xl border bg-widget opacity-50 text-[10px]">Error: ${r.location}</div>`;
  ...
  document.querySelectorAll('.weather-widget-card').forEach(card => {
    card.onclick = () => window.open(`https://www.wunderground.com/weather/${card.dataset.lat},${card.dataset.lng}`, '_blank');
  });
};
...
const attachAppEvents = () => {
  ...
  if (nInput) nInput.oninput = (e) => localStorage.setItem('speeddial_quicknote', e.target.value);

  document.querySelectorAll('.tile-link').forEach((link, idx) => {
    link.onclick = () => {
      const pageTiles = state.tiles.filter(t => t.pageId === state.activePageId).sort((a,b)=>a.position-b.position);
  ...
  if (grid) {
    Sortable.create(grid, {
      ...
      onEnd: (evt) => {
        ...
        state.tiles = [...state.tiles.filter(t => t.pageId !== state.activePageId), ...pageTiles];
        saveState();
      }
    });
  }
...
const openEditModal = (tileId) => {
  const tile = tileId ? state.tiles.find(t => t.id === tileId) : { title: '', url: '', imageUrl: '', pageId: state.activePageId };
  const isDark = state.settings.theme === 'dark';
  ...
  if (tileId) document.getElementById('form-delete').onclick = () => {
    state.tiles = state.tiles.filter(t => t.id !== tileId);
    saveState(); modalRoot.innerHTML = ''; render();
  };
  document.getElementById('form-save').onclick = () => {
    ...
    if (tileId) {
      ...
      state.tiles.push({ id: 't' + Date.now(), url, title, imageUrl, pageId, position: state.tiles.length });
    }
    saveState(); modalRoot.innerHTML = ''; render();
  };
};
      ...
      <div class="w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden border animate-zoom-in ${isDark ? 'bg-slate-900 border-slate-800 text-slate-100' : 'bg-white border-gray-100 text-gray-900'}">
        ...
        <div class="p-6 space-y-8 overflow-y-auto max-h-[70vh] no-scrollbar">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-3">
              ...
              <div class="flex flex-wrap gap-2 items-center p-3 rounded-xl border dark:border-slate-800 bg-current/5">
                ${['#f97316', '#3b82f6', '#10b981', '#ef4444', '#8b5cf6', '#ec4899', '#06b6d4'].map(c => `
            ...
            <div class="space-y-3">
              ...
              <div class="flex flex-wrap gap-2 items-center p-3 rounded-xl border dark:border-slate-800 bg-current/5">
                ${['#020617', '#1e293b', '#000000', '#f3f4f6', '#ffffff', '#111827'].map(c => `
          ...
          <div class="space-y-4">
            ...
            <div class="space-y-2">
              ${state.settings.weatherConfigs.map(c => `
...
const attachSettingsEvents = () => {
  ...
  document.getElementById('s-tab').onclick = () => { state.settings.openInNewTab = !state.settings.openInNewTab; openSettings(); };
  document.getElementById('s-weather-toggle').onclick = () => { state.settings.showWeather = !state.settings.showWeather; openSettings(); };
  document.getElementById('s-notes-pos').onclick = () => { state.settings.notesPosition = state.settings.notesPosition === 'left' ? 'right' : 'left'; openSettings(); };

  document.querySelectorAll('.color-pick').forEach(b => b.onclick = () => { 
    state.settings.themeColor = b.dataset.color; 
    openSettings(); 
  });
  document.querySelectorAll('.bg-pick').forEach(b => b.onclick = () => { 
  ...
  if (bgUrlInput) {
    bgUrlInput.oninput = (e) => {
      ...
      if (previewBox) previewBox.style.backgroundImage = state.settings.backgroundImage ? `url(${state.settings.backgroundImage})` : 'none';
      const container = document.getElementById('main-container');
      if (container) container.style.backgroundImage = state.settings.backgroundImage ? `url(${state.settings.backgroundImage})` : 'none';
    };
  }
  ...
  if (bgUploadBtn && bgFileInput) {
    ...
    bgFileInput.onchange = (e) => {
      ...
      if (file.size > 4500000) {
        alert("This file is too large to save locally. Please use an image under 2MB or host the file online and use its URL.");
        return;
      }
      ...
      reader.onload = (event) => {
        ...
        const previewBox = document.getElementById('bg-preview-box');
        if (previewBox) previewBox.style.backgroundImage = `url(${state.settings.backgroundImage})`;
        const container = document.getElementById('main-container');
        if (container) container.style.backgroundImage = `url(${state.settings.backgroundImage})`;
      };
    ...
    cityInput.oninput = (e) => {
      ...
      searchTimer = setTimeout(async () => {
        try {
          ...
          const data = await res.json();
          if (data.results) {
            cityResults.innerHTML = data.results.map(loc => `
            ...
            document.querySelectorAll('.city-opt').forEach(opt => {
              opt.onclick = () => {
                state.settings.weatherConfigs.push({ id: Date.now(), location: opt.dataset.name, lat: parseFloat(opt.dataset.lat), lng: parseFloat(opt.dataset.lng) });
                openSettings();
              };
            });
          }
  ...
  document.querySelectorAll('.city-del').forEach(b => b.onclick = () => {
    state.settings.weatherConfigs = state.settings.weatherConfigs.filter(c => c.id != b.dataset.cityId);
    openSettings();
  });

  document.getElementById('s-add-group').onclick = () => { state.pages.push({ id: 'p' + Date.now(), name: 'New Group' }); openSettings(); };
  document.getElementById('s-sort-az').onclick = () => { state.pages.sort((a,b) => a.name.localeCompare(b.name)); openSettings(); };
  document.querySelectorAll('.group-del').forEach(b => b.onclick = () => {
  ...
  document.getElementById('s-export').onclick = () => {
    const data = JSON.stringify({ settings: state.settings, tiles: state.tiles, pages: state.pages });
    const blob = new Blob([data], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = 'speeddial-native-backup.json'; a.click();
    URL.revokeObjectURL(url);
  };
  
  const fileInput = document.getElementById('import-file');
  document.getElementById('s-import').onclick = () => { fileInput.dataset.mode = 'native'; fileInput.click(); };
  document.getElementById('s-import-sd2').onclick = () => { fileInput.dataset.mode = 'sd2'; fileInput.click(); };
    ...
    reader.onload = (event) => {
      try {
        ...
        if (fileInput.dataset.mode === 'sd2') {
          const sd2Pages = (json.groups || []).map(g => ({ id: String(g.id), name: g.title }));
          const sd2Tiles = (json.dials || []).map(d => ({
            id: String(d.id || Math.random()), title: d.title, url: d.url, imageUrl: d.thumbnail || '', position: d.position, pageId: String(d.idgroup)
          }));
